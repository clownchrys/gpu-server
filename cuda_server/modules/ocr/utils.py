        ...
        if len(boxes) == 1:
            ...
            for box in boxes:
                if len(new_box) == 0:
                    ...
                    cond1 = abs(np.mean(b_height) - box[5]) < height_ths * np.mean(b_height)
                    cond2 = abs(box[0] - x_max) < width_ths * (box[3]-box[2])
                    if cond1 and cond2:
...
def reformat_input(image):
    if type(image) == str:
        ...
        raise TypeError('Invalid input type. Suppoting format = string(file path or url), bytes, numpy array')
...
def compat_tr(coords):
    arr = np.array(coords)
    return [
        *arr.mean(0).astype(float).tolist(),
        *(arr.max(0) - arr.min(0)).astype(float).tolist(),
        0.
    ]